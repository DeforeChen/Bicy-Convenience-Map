...
+(NSString*)DesWithSrcString:(NSString*)plaintStr KeyString:(NSString*)keyStr {
    ...
    unsigned char ucDes[8];
    
    // 压缩Hex， 31 -> "\x31"
    NSData *plaintData = [NLTransLib hexToBytes:plaintStr];
    const char *p_Char = [plaintData bytes];
    memcpy(ucSrc, p_Char, 8);
    
    NSData *keyData    = [NLTransLib hexToBytes:keyStr];
    const char *k_Char = [keyData bytes];
    memcpy(ucKey, k_Char, 8);
    
    DES(ucSrc, ucDes, ucKey, 1);
    
    NSMutableString *DesStr = [[NSMutableString alloc] init];
    for (int i = 0; i < 8; i++) {
        NSString *byteStr = [NSString stringWithFormat:@"%02x",ucDes[i]];
        [DesStr appendString:byteStr];
    }
    
    return [DesStr uppercaseString];
}
...
+(NSString*)ThreeDesWithSrcString:(NSString*)plaintStr KeyString:(NSString*)keyStr {
    ...
    NSData *keyData    = [NLTransLib hexToBytes:keyStr];
    const char *k_Char = [keyData bytes];
    memcpy(ucKey, k_Char, 16);
    
    DES3_encrypt(ucSrc, ucKey, ucDes);
